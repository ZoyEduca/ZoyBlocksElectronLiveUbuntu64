 ...
 * Version             : v0.8.2(Otimização Servo.h e NewPing)
 * Date                : 11/12/2025
...
#include <NewPing.h> // Biblioteca eficiente para Ultrassom

// ==== CONFIGURAÇÕES GERAIS ====
const int LED_13 = 13;
const int BUZZER = 12;
const int ZOY_MAX_SERVOS = 6;     // Quantidade máxima de servos simultâneos
const int MAX_DISTANCIA = 200; // Distância máxima ultrassom (cm) para evitar lags
...
void pararServo(int pino);

// === LEITURA ULTRASSOM (NewPing) ===
float ler_ultrassom(int trigPin, int echoPin) {
  // Instancia localmente para permitir pinos dinâmicos
  NewPing sonar(trigPin, echoPin, MAX_DISTANCIA);
  
  // ping_cm() retorna 0 se fora de alcance, convertemos para float
  unsigned int distancia = sonar.ping_cm();
  
  // Pequeno delay para estabilidade se chamar muito rápido, NewPing gerencia bem
...
void setup() {
  ...
  Serial.println("ZOY_FIRMWARE_V0.8.2");
